...
* 用于python环境的玩具和容器，vless+trojan+shadowsocks三协议，内嵌 Python 版哪吒客户端(v0或v1)，可自行添加环境变量开启，不再运行时下载外部 agent 二进制
...
* PaaS 平台设置的环境变量
  ...
  | NEZHA_KEY    | 否 |        | 哪吒v1的NZ_CLIENT_SECRET或v0的agent密钥 |
...
* 哪吒客户端已内嵌在 `app.py` 中，启用 `NEZHA_SERVER` + `NEZHA_KEY` 后随主程序启动；面板终端功能使用容器内 `/bin/bash` 或 `/bin/sh`，请妥善保护 `NEZHA_KEY`
